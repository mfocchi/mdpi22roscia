...
\Author{Francesco Roscia $^{1, ^*}$, Andrea Cumerlotti $^{?}$, Andrea Del Prete $^{2}$, Claudio Semini $^{1}$ and Michele Focchi$^{1, \, 3}$}
...
\corres{Correspondence: francesco.roscia@iit.it}
...
\begin{figure}[t!]
	\centering
	\includegraphics[width=.5\linewidth]{figures/solo_fw_pic_compressed.png}
...
\FR{In Solo12, even if the each limb accounts for about $13.4\%$ of the total mass, changes of the total robot inertia come with the motion of only the upperleg (thigh) and the lowerleg (calf), since the displacement of distribution of the hip mass with the system \acrshort{com} is approximately constant. In view of this, each limb accounts for only $7.5\%$ of the total mass in varying the the total robot inertia.}
...
\begin{table}[H] 
	\caption{Size and dynamic parameters of a single flywheel}
	\label{tab:dim_fw}
