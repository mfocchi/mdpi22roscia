...
\Author{\hl{Francesco} %MDPI: Please carefully check the accuracy of names and affiliations.
...
\address{%
$^{1}$ \quad Dynamic Legged Systems (DLS) Lab, Istituto Italiano di Tecnologia (IIT), \hl{Genoa,} %MDPI: Please add the postal code (or ZIP code in the U.S.). If the postal code is not available, please provide the P.O. Box. The following highlights are the same.
 Italy; \linebreak  \hl{francesco.roscia@iit.it} %MDPI: We added all the emails in affiliations according to the submitting system. Please confirm.
 (F.R.); andrea.cumerlotti@studenti.unitn.it (A.C.); claudio.semini@iit.it (C.S.)\\
$^{2}$ \quad Industrial Engineering Department (DII), University of Trento, \hl{Trento}, Italy; andrea.delprete@unitn.it (A.D.P.)\\
$^{3}$ \quad Department of Information Engineering and Computer Science (DISI), University of Trento, \hl{Trento}, Italy\\
}
...
\begin{itemize}
	...
	In particular, the axes of rotation of the flywheels are set to be incident, enabling \hl{\textit{continuous}} %MDPI: Please confirm if all the italics in the maintext should be retained.
...
The starting point for any OCS is the Euler's equation. For any mechanical system, the time derivative of the angular \hl{momentum}, %MDPI: Please confirm if all the bold in equations should be retained.
...
\end{equation}
\hl{When} %MDPI: Please confirm if the noindent should be retained. The following highlights are the same.
...
\vspace{-6pt}
\begin{table}[H]%[t] 1
	\caption{\hl{Sizes} %MDPI: Please confirm if all the "\cdot" between numbers should be a multiplication sign ("\times" U+00D7).
...
\end{equation}
\hl{The} OCS was commanded to track base orientation references using the control law introduced in Section \ref{subsec:control_law} \MF{coded in Python}. ({\hl{The} %MDPI: Footnote is not permitted in our journal. We include this paragraph to the maintext. Please confirm.
 video, with all the simulations, is available at web link \url{https://www.dropbox.com/scl/fo/wevvl0zvfp8gofie6x7rw/h?dl=0&rlkey=pap76cr813a0skrvrxl7qgrb9}  (\hl{accessed} %MDPI: Please add the access date (Format: Date Month Year). e.g., (accessed on 1 January 2020).
 on).)}
...
\newblock In Proceedings of the 2013 IEEE International Conference on Robotics
  and Automation, \hl{Karlsruhe, Germany, 6--10 May 2013;} %MDPI: Newly added information. Please confirm. The following highlights are the same.
...
\newblock Heuristic planning for rough terrain locomotion in presence of
  ...
  Robotics Research: From Lab to Market}; Springer: \hl{Berlin/Heidelberg, Germany,} %We added the location of publisher. Please confirm
  2020; pp. 165--209.
...
\newblock {\em IEEE/ASME Trans. Mechatron.} {\bf 2022}, \hl{\emph{7}, 5072--5083}.
...
\newblock \emph{Open Robot Actuator Hardware}\hl{; .} %MDPI: Please add the name of the publisher and the location of it.
