...
For legged robots, aerial motions are the only option to overpass obstacles that cannot be circumvent with standard locomotion gaits. In these cases, the robot must perform a leap to position itself on the obstruction or even flyover it. However, these movements represent a challenge because during the flight phase the center of mass cannot be controlled, and the robot orientation has limited controllability. We focus on the latter issue and propose an \gls{ocs} consisting of two rotating and actuated masses (named flywheels or reaction wheels) to gain control authority on the robot orientation. Because of the conservation of angular momentum, their rotation can be adjusted to steer the robot orientation even when there are no contacts with the ground. The axes of rotation of the flywheels are designed to be incident, leading to a compact orientation control system that is capable of controlling both roll and pitch angles, considering the different moment of inertia in the two directions. We tested the concept with simulations and experiments on the real robot Solo12~\cite[solo].
\end{abstract}
...
Different types of gait, such as trot or crawl, have been developed for quadrupedal robots. 
Thanks to the progress of the last two decades, robots have become lighter and stronger, which has enabled them to perform highly dynamic maneuvers.
However, sometimes it is not possible to get around an obstacle with the gaits mentioned above, and jumps are required. 
...
\begin{itemize}
	\item a novel design that enables to control the orientation of a legged robot during a jump in an effective way, while keeping the design complexity simple. 
...
\end{equation}
When the resultant of the external moments applied to the system is zero, Euler’s equation simplifies in
\begin{equation}
...
In this case, it is possible to change the angular velocity of this base, changing the joints position and velocity, as a result of the \textit{non-holonomy} of the total angular momentum \cite{Wieber16}: if the angular momentum of a certain body changes, the one of the others must modify to maintain the total sum constant.
...
Lets identify the moment of inertia of the robot in the nominal configuration as $I_r$ and the moment of inertia of the two flywheels as $I_f$, both referred to system \gls{com}.
...
\begin{enumerate}
	\item use two couples of flywheels, each couple having the rotation axes aligned and orthogonal to the ones of the other couple \MF{metti foto con i due design uno di fianco all altro};
...
\begin{figure}
	...
	\label{fig:axes}
\end{figure}
The angle is selected considering the ratio of the eigenvalues along the $x$ and $y$ directions of the ellipsoid of robot inertia:
\begin{equation*}
...
\end{split}
\end{equation*}
We can use the PD action $\bm{K}_p \bm{e} + \bm{K}_d \dot{\bm{e}}$ to generate a reference acceleration for the base $\dot{\bm{\omega}}_b$.$\bm{K}_p$ and $\bm{K}_d$ are diagonal and positive-definite gain matrices for the error in attitude and angular velocity.
...
track base orientation references using the control law introduced in \ref{subsec:control_law}. The video with all the simulations is available at web link \FR{metti link a dropbox}.
...
During the flight phase of a forward jump, $0.1 \mathrm{s}$ after the lift off, an external disturbance moment on the robot base is applied that deflects the robot orientation: this disturbance is set to $\dot{\bm{L}}_{b, dist} = \left[\begin{array}{ccc}
0.2 & 0.8 & 0.0
\end{array}
\right]^T \ \mathrm{N \cdot m}$ and applies for $0.05 \ \mathrm{s}$. 
If the flywheels are kept stopped, Solo12 falls after the touch down. Instead, 
keeping the flywheels active  %pairing the legs joint PD controller with \ref{eq:ctrl_law},
...
We let the robot falling from a height of $1 \ \mathrm{m}$ with an initial pitch orientation of $30^\circ$ and zero base angular velocity.
If we do not actuate the reaction wheels, the robot touches the ground with the same initial orientation and the trunk oscillates. If the \gls{ocs} drives the robot base to be horizontal when it is still in air, the oscillations after the touch down are drastically reduced, both in the pitch angle and forward direction. \FR{add a plot}
...
Finally, we demonstrate how reaction wheels can alleviate the effort of legs joints to achieve a highly dynamic motion: a back-flip. 
For this, we target a space application carrying out a simulation with Moon gravity ($1.62 \ \mathrm{m/s^2}$).
In this way, it is possible to obtain high jumps with a long flight phase, allowing us to achieve the task without having to select more powerful actuators.  
...
Our design allows the manual change of this value before performing the task.
