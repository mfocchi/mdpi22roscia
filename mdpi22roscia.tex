...
\end{equation}
\ADP{You forgot the dependency on $I_r$}
...
\begin{figure}
	...
	\caption{\small Minimum inertia necessary to obtain the angular velocity $\dot{\theta}_{des}$ starting with different initial $\dot{\theta}_0$ conditions. The area between the two red lines represents what can be achieved with our \gls{ocs}. \ADP{I would rather see a plot having as axes $\Delta \dot \theta = \dot \theta_{des}- \dot \theta_0$ and $\dot \gamma^{max}$. Moreover the plot should display $I_f / I_r$ because there's an error in the equation below.}}
...
\begin{enumerate}
	\item use two couples of flywheels, each couple having the rotation axes aligned and orthogonal to the ones of the other couple \MF{metti foto con i due design uno di fianco all altro}; \ADP{Not clear why you would need 4 flywheels and not just 2. Also, I think it's not interesting to explain this design if then we don't use it.}
	\item use two flywheels, with incident axes of rotation.
\end{enumerate}
The first case would lead to a bulky \gls{ocs}, requiring brakes to achieve relevant angular momentum. Thus, the second option is considered. The inertia introduced in the Elroy's Beanie model has to be split between the two reaction wheels: $I_f = 2 \, I_{f, zz}$. \ADP{Not clear why the inertia should be split because you haven't discussed yet the fact that the flywheels won't be aligned with the axes. Move this to a later point.}
